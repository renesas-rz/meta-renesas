...
bindir_progs = "arch basename chcon cksum comm csplit cut dir dircolors dirname du \
                ...
                pinky pr printenv printf ptx realpath runcon seq sha1sum sha224sum sha256sum \
...
do_install_append() {
	for i in df mktemp base64 readlink; do mv ${D}${bindir}/$i ${D}${bindir}/$i.${BPN}; done
...
pkg_postinst_${PN} () {
	update-alternatives --install ${base_bindir}/readlink readlink ${bindir}/readlink.${PN} 100;
}

pkg_prerm_${PN} () {
	update-alternatives --remove readlink ${bindir}/readlink.${PN}
}
