...
if [ -z "$SGX_KM" ]; then
	echo "Error: can not find *SGX_KM_M2.tar.bz2"
	exit -1
fi
cp -r $SGX_KM recipes-kernel/gles-module/kernel-module-gles/SGX_KM_M2.tar.bz2

SGX_LIB=`find $1 -name *r8a7743_linux_sgx_binaries_gles2.tar.bz2 | tail -1`
if [ -z "$SGX_LIB" ]; then
	echo "Error: can not find *r8a7743_linux_sgx_binaries_gles2.tar.bz2"
	exit -1
fi
cp -r $SGX_LIB recipes-graphics/gles-module/gles-user-module/r8a7743_linux_sgx_binaries_gles2.tar.bz2
