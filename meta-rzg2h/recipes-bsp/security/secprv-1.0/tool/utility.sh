...
func_extract_rsaprm ()
{
	...
	if [ 0 = $? ]; then
		txt_rsa_key="$(echo "${pem_rsa_key}" | openssl rsa -noout -text)"
	else
		txt_rsa_key="$(echo "${pem_rsa_key}" | openssl rsa -noout -text -pubin)"
	fi
...
#*******************************************************************************
# Function Name: func_load_pubkey
...
#*******************************************************************************
# Function Name: func_load_prvkey
